...
module ComputerTurn
    ...
    def card_score(player_array, card, play_array)
        ...
        when "Pickup 4"
            ...
            #next opponent has less than 3 cards
            if player_array[1].cards.length < 3
                card.score = pickup_4_score + 70
            #only wild can play && wild outnumber different colors
            elsif num_basic_colors_can_play == 0 && play_array.length >= num_diff_colors 
                card.score = pickup_4_score + 70
            elsif num_basic_colors_can_play == 0 && play_array.length < num_diff_colors
                return false
            else
                card.score = pickup_4_score + player_array[0].has_number?("Pickup 4")
            end
        when "Card"
            num_basic_colors_can_play = number_different_colors(player_array[0], play_array)
            num_diff_colors = number_different_colors(player_array[0], player_array[0].cards)
            #next player has only 1 card left and played a wild last turn
            if player_array[1].cards.length == 1 && player_array[1].last_action == "Wild"
    ...
    def computer_3(player_array, match_card, deck, play_array)
        ...
        num_diff_colors = number_different_colors(player_array[0], player_array[0].cards)
        # if player_array[1].cards.length == 1 && player_array[1].last_action == "Wild"
        #     if player_array[0].has_card("Wild", "Card")
        #         chosen_card = player_array[0].has_card("Wild", "Card")
        #         return chosen_card
        #     end
        # end
        #colors in hand outnumber wild cards - draw card
        num_basic_colors_can_play = number_different_colors(player_array[0], play_array)
